...
void PostProcessorFluxMap::compute_flux_analysis() {
  for (size_t j = 0; j < list_of_analyzed_species_.size(); j++) {
    const size_t index_j = list_of_analyzed_species_[j];
    compute_values(index_j, global_important_indices_[index_j], global_relative_thickness_[index_j],
    ...
    for (size_t k = 0; k < global_important_indices_[index_j].size(); k++) {
      index_first_name.push_back(index_j);
      index_second_name.push_back(global_important_indices_[index_j][k]);
    }
  }
}

void PostProcessorFluxMap::compute_values(size_t index_j, std::vector<unsigned int> &local_indices,
                                          std::vector<double> &local_thickness,
                                          std::vector<double> &local_normal_fluxes, std::vector<double> &local_fluxes) {
  for (size_t j = 0; j < local_indices.size(); j++) {
    if (normal_tags_ == true) {
      ...
      computed_label_value.push_back(local_normal_fluxes[j]);
    } else {
      std::setprecision(2);
      computed_label_value.push_back(local_fluxes[j]);
    }
    // Thickness
    double thickness;
    ...
    computed_thickness_value.push_back(thickness);
  }
}
}  // namespace pySMOKEPostProcessor
