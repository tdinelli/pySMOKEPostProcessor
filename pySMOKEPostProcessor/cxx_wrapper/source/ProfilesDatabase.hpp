...
bool ProfilesDatabase::ReadKineticMechanism(const std::string &folder_name) {
  ...
  // Read from file
  {
    ...
    kineticsMapXML =
        new OpenSMOKE::KineticsMap_CHEMKIN(*thermodynamicsMapXML, ptree, false);
  }
  ...
  if (thermodynamicsMapXML->NumberOfSpecies() == omega.size()) {
    ...
    throw std::invalid_argument(
        "Output.xml file contains only a subset of the total species in the kinetic "
        "mechanism");
  }
  ...
  // Read the reaction strings
  {
    ...
    if (!boost::filesystem::exists(path_reaction_names)) {
      throw std::invalid_argument(
          "Kinetic folder does not contain the reaction_names.xml file");
    }
...
void ProfilesDatabase::Prepare() {
  ...
  std::vector<std::string> string_list_massfractions_unsorted;
  {
    ...
    if (child) {
      ...
      std::sort(string_list_massfractions_sorted.begin(),
                string_list_massfractions_sorted.end());
      ...
      for (unsigned int j = 0; j < number_of_massfractions_profiles; j++)
        for (unsigned int k = 0; k < number_of_massfractions_profiles; k++)
          if (string_list_massfractions_sorted[j] ==
  ...
  {
    {
      ...
      if (child) {
        ...
        throw std::invalid_argument(
            "Corrupted xml file: missing the profiles-size leaf");
      }
    }
    ...
    for (unsigned int j = 0; j < column_index_of_massfractions_profiles.size(); j++) {
      sorted_max[j] = -1.e100;
      for (unsigned int i = 0; i < number_of_abscissas_; i++)
        if (omega[sorted_index[j]][i] > sorted_max[j])
          sorted_max[j] = omega[sorted_index[j]][i];
    }
  }
  ...
  // Conversions
  {
    for (unsigned int j = 0; j < column_index_of_massfractions_profiles.size(); j++) {
      if (omega[j][0] > 1e-8) {
        ...
        string_list_additional.push_back("conversion-" +
                                         string_list_massfractions_unsorted[j]);
...
void ProfilesDatabase::ReactionsAssociatedToSpecies(const unsigned int index,
  ...
  for (int k = 0;
       k < kineticsMapXML->stoichiometry().stoichiometric_matrix_reactants().outerSize();
       ++k) {
  ...
  for (int k = 0;
       k < kineticsMapXML->stoichiometry().stoichiometric_matrix_products().outerSize();
       ++k) {
    for (Eigen::SparseMatrix<double>::InnerIterator it(
