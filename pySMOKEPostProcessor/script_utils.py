...
def process_classes(simul_fld, kin_xml_fld, rxns_sorted_obj, species_list, sortlists, ropa_type, n_of_rxns=100,
    ...
    if isinstance(species_list[0], dict):
        flat_species_list = sum([list(d.values())[0]
...
def cumulative_rates(simul_fld, kin_xml_fld, species_list, rate_type, x_axis,
    ...
    """
    Cumulative rates: dictionary with dataframes of cumulative reaction rates for species then ready to plot
    ...
    for species in species_list:
        tot_rop_dct = pp.RateOfProductionAnalysis(species,
                                                  ropa_type='global',
...
def reactionrates_byclasses(simul_fld, kin_xml_fld, rxns_sorted_df, sortlists, x_axis,
                            filter_by_species=[], filter_dcts=None, threshs=None, mass_ropa=False):
    """
