...
class GraphWriter:
    ...
    def CreateGraph(self, edgeStart, edgeEnd, thickness, label):
        ...
        """
        # There is a possible bug I need to do a check on list edgeStart edgeEnd intersection cause it can generate bugs
        # when for instance end nodes are just accumulation point
    ...
    def AddNodes(self, NodesNames) -> None:
        """
    ...
    def AddEdges(self, edgeStart, edgeEnd, tck, edgeLabel) -> None:
        """
        ...
        for i, start in enumerate(edgeStart):
            ...
            if (self.fluxAnalysisType_ == 'destruction'):
                self.G.edge(map_name_start, map_name_end, color=self.color,
                            weight=str(tickness), label=label_str, penwidth=str(tickness))
            elif (self.fluxAnalysisType_ == 'production'):
                # Arrows have to point the opposite direction
                self.G.edge(map_name_end, map_name_start, label=label_str,
                            color=self.color, weight=str(tickness), penwidth=str(tickness))
