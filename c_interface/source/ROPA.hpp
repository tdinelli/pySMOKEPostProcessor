...
void ROPA::RateOfProductionAnalysis(double* coefficients, int* reactions, int len)
{
	...
	if (std::find(data_->string_list_massfractions_sorted.begin(), data_->string_list_massfractions_sorted.end(), species_) != data_->string_list_massfractions_sorted.end()){
		...
		throw PostProcessor::Exception("Please select one of the available species!");
	}
    ...
    if (ropaType_ == "local"){
        ...
        for (unsigned int j = index_min; j < index_max - 1; j++){
            ...
            {
				throw PostProcessor::Exception("SSS");
            }
...
void ROPA::GetFormationRates(std::string specie, std::string units, std::string type, double* rate){
...

	{
		...
		for(unsigned int j=0;j<n_selected_species;j++){
			for(unsigned int k=0;k<data_->string_list_massfractions_sorted.size();k++){ // CHECKKCKKKC
